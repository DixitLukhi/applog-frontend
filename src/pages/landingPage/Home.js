              ...
              <tbody>
                {oneAppData?.guidelines.map((pol) => (
                  <tr key={pol._id._id} className={`policy-item ${pol?._id?.priority === 'l' ? 'bg-green' : pol?._id?.priority === 'h' ? 'bg-red' : 'bg-yellow'}`}>
                    ...
                    <div onClick={() => {setProofData({proofid: (oneAppData?.appName + "_" + pol?._id?.policyid).replace(/ /g, "_").toUpperCase(), img: pol?.proofImage?.url, desc: pol?.proofDescription}); setIsProofPopUpOpen(true)}}> {pol.followed ?
